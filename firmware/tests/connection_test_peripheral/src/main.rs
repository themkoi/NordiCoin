...
async fn main(spawner: Spawner) {
    ...
    // SDC memory: same as ble_bas_peripheral
    ...
    // Create GATT server with custom service
    ...
    let _ = join(ble_task(runner), async {
        loop {
            match advertise("Nordicoin", address, &mut peripheral, &server).await {
                Ok(conn) => {
                    // Set up tasks when connection is established
...
/// Background BLE task that must run forever
...
/// Advertise and wait for a central to connect using extended advertising
...
/// Handle GATT events (read/write requests from central)
            ...
            GattConnectionEvent::Gatt { event } => {
                let reply = match event {
                    ...
                    GattEvent::Write(event) => {
                        if event.handle() == status_handle {
                            // Status is read-only, reject writes
...
/// Background task that notifies the status counter every 2 seconds
async fn notify_task<C: Controller, P: PacketPool>(
    ...
    let mut tick: u16 = 0;
    let status = &server.test_service.status;
    loop {
        tick = tick.wrapping_add(1);
        info!("[notify] notifying status = {}", tick);
        if status.notify(conn, &tick, true).await.is_err() {
            info!("[notify] notification failed, connection may be closed");
            break;
        }
        // Read RSSI
        if let Ok(rssi) = conn.raw().rssi(stack).await {
            info!("[notify] RSSI: {:?}", rssi);
        }
